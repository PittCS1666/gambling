...
use std::{thread, time};
mod update_events;
use update_events::{PlayerUpdateEvent, PokerTurnUpdateEvent, CommunityCardsUpdateEvent, LastPlayerActionUpdateEvent,
                    NumPlayersUpadteEvent, PokerPhaseUpdateEvent, init_player_update_channel, init_poker_turn_update_channel, 
                    init_community_cards_update_channel, init_last_player_action_update_channel, init_num_players_update_channel, 
                    init_poker_phase_update_channel};
...
/*
pub fn serialize_player(player: &Player) -> String {
    serde_json::to_string(player).expect("Failed to serialize the player");
}

pub fn serialize_poker_turn(poker_turn: &PokerTurn) -> String {
    serde_json::to_string(poker_turn).expect("Failed to serialize the poker turn");
}

pub fn serialize_community_cards(community_cards: &CommunityCards) -> String {
    serde_json::to_string(community_cards).expect("Failed to serialize the community cards");
}
...
fn process_player_turn(
    ...
    // added event channels as parameters so they can be updated
    player_update_channel: &EventChannel<PlayerUpdateEvent>,
    poker_turn_update_channel: &EventChannel<PokerTurnUpdateEvent>,
    community_cards_update_channel: &EventChannel<CommunityCardsUpdateEvent>,
    last_player_action_update_channel: &EventChannel<LastPlayerActionUpdateEvent>,
    num_players_update_channel: &EventChannel<NumPlayersUpdateEvent>,
    poker_phase_update_channel: &EventChannel<PokerPhaseUpdateEvent>,
)  {
    ...
    //update
    player_update_channel.single_write(PlayerUpdateEvent(state.clone()));
    poker_turn_update_channel.single_write(PokerTurnUpdateEvent(state.clone()));
    community_cards_update_channel.single_write(CommunityCardsUpdateEvent(state.clone()));
    last_player_action_update_channel.single_write(LastPlayerActionUpdateEvent(state.clone()));
    num_players_update_channel.single_write(NumPlayersUpdateEvent(state.clone()));
    poker_phase_update_channel.single_write(PokerPhaseUpdateEvent(state.clone()));
